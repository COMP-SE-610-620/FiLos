...
const VoiceRecorder = ({ sendingMessage, handleFileUpload }) => {
  ...
  const AUDIO_UPLOAD_DEBUG = false;
    ...
    try {
      ...
      recorder.onstop = () => {
        ...
        fetch('http://localhost:8000/chat/speech', {
          ...
          .then(data => {
            setLoading(false);
            console.log('Response for Speech-to-text:', data.response);
...
class TextBar extends React.Component {
  ...
  sendMessage(prompt, TTS) {
    this.props.onSend && this.props.onSend(prompt, TTS);
    this.input.current.value = '';
  }

  sendMessageIfEnter = (e) => {
    if (e.keyCode === 13) {
      this.sendMessage(this.input.current.value, false);
    }
  };
